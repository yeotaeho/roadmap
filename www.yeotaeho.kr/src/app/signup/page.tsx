// SNS 전용 회원가입 화면 — 목표 직무·관심 키워드는 가입 이후 AI 코치에서 탐색
"use client";
...
export default function SignupPage() {
    ...
    const handleSocialLogin = async (provider: 'kakao' | 'naver' | 'google') => {
        try {
            const response = await fetch(`${API_BASE}/api/oauth/${provider}/login?mode=signup`, {
                method: 'GET',
                credentials: 'include',
                headers: {
        ...
        <div className="min-h-screen bg-background flex items-center justify-center px-4 py-16">
            <div className="w-full max-w-md">
                ...
                <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
                    <h1 className="text-2xl font-bold text-center text-foreground mb-2">회원가입</h1>
                    <p className="text-center text-sm text-muted-foreground mb-8">
                        SNS 계정으로 3초 만에 시작하세요. 목표 직무와 관심 키워드는
                        가입 후 AI 코치와 함께 찾아드려요.
                    </p>
