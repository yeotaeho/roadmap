...
import { useGapIssueDetail } from "@/hooks/useDashboard";

export default function GapIssueDetailPage() {
  const params = useParams();
  const id = String(params?.id ?? "");
  const { data: issue, isLoading, isError } = useGapIssueDetail(id);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">discourse에서 추출한 결핍/기회를 확장해 설명합니다.</p>
      </div>

      <PanelStatus isLoading={isLoading} isError={isError} isEmpty={!issue} label="이슈">
        {issue && (
          <div className="space-y-6">
            <article className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm dark:border-slate-700">
              <div className="grid sm:grid-cols-2">
                <div className="bg-slate-900 text-white p-5">
                  ...
                  <p className="mt-2 text-base font-semibold leading-relaxed">{issue.problem_summary}</p>
                </div>
                <div className="bg-emerald-50 p-5 border-t sm:border-t-0 sm:border-l border-emerald-100 dark:bg-emerald-900/20 dark:border-emerald-900/40">
                  ...
                  <p className="mt-2 text-base font-semibold text-emerald-950 leading-relaxed dark:text-emerald-200">{issue.chance_summary}</p>
                </div>
              </div>
            </article>

            {issue.detail_summary && (
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">요약</h2>
                <p className="mt-3 text-sm text-slate-700 leading-relaxed dark:text-slate-300">{issue.detail_summary}</p>
              </section>
            )}

            <div className="grid gap-4 lg:grid-cols-2">
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">이해관계자</h2>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">다음 액션</h2>
                <ol className="mt-3 space-y-2 text-sm text-slate-700 list-decimal pl-5 dark:text-slate-300">
                  {issue.next_actions.map((s) => (
                    <li key={s}>{s}</li>
                  ))}
                </ol>
              </section>
            ...
            {issue.evidences.length > 0 && (
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">근거</h2>
                <ul className="mt-3 space-y-2">
                  {issue.evidences.map((ev, idx) => (
                    <li key={`${ev.title}-${idx}`} className="text-sm">
                      {ev.url ? (
                        <a
                          href={ev.url}
                          className="font-medium text-indigo-600 hover:text-indigo-700 underline underline-offset-2 dark:text-indigo-300"
                          target="_blank"
                          rel="noreferrer"
                        >
                          {ev.title}
