...
async def list_sessions(
    ...
    """사용자의 상담 대화 세션 목록(메시지 있는 세션, 최신순)."""
    sessions = await ConsultService(db).list_sessions(user_id)
    return {
        "success": True,
        "sessions": [
...
async def create_new_session(
    ...
    """항상 새 상담 세션 생성(기존 active 재사용 안 함)."""
    session_id = await ConsultService(db).create_new_session(user_id)
    return {"success": True, "sessionId": session_id}
