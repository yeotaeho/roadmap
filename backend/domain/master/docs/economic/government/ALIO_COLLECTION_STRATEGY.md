...
| **Collector 경로** | `hub/services/collectors/economic/alio/alio_public_inst_project_collector.py` |
...
**제안 지표**: (a) 파싱 실패율 로그 카운트, (b) `investment_amount` NULL 비율, (c) `alio://project/` 합성 식별자 비율(원문 URL·사업 ID 미제공 품질 신호).
