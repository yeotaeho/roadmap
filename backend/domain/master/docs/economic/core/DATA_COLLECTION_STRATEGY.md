...
> **현재 구현 주의**
>
> 이 문서에는 초기 목표 구조와 예시 코드가 함께 남아 있다.
> 현재 코드 구조, API, 스케줄, 물리 적재 테이블은
> [`MASTER_BRONZE_IMPLEMENTATION_STATUS.md`](./MASTER_BRONZE_IMPLEMENTATION_STATUS.md)를 우선한다.
> 현재 Collector 경로는 `spokes/collectors`가 아니라
> `hub/services/collectors`이며 스케줄러는 `backend/core/scheduler.py`에 있다.
...
목표 모델은 5개 원천 테이블이지만 현재 master 수집 파이프라인은
`raw_economic_data`, `raw_market_timeseries`, `raw_opportunity_data` 3개를 직접 사용합니다.
KIPRIS와 Naver DataLab도 현재는 `raw_economic_data`에 적재합니다.

```
backend/domain/master/
├── hub/
│   ├── repositories/
│   │   ├── economic_repository.py
│   │   ├── market_timeseries_repository.py
│   │   └── opportunity_repository.py
│   └── services/
│       ├── bronze_economic_ingest_service.py
│       ├── bronze_market_timeseries_ingest_service.py
│       ├── bronze_opportunity_ingest_service.py
│       └── collectors/
│           ├── economic/
│           └── opportunity/
└── models/
    ├── bases/
    │   ├── raw_economic_data.py
    │   ├── raw_market_timeseries.py
    │   └── raw_opportunity_data.py
    └── transfer/                    # Pydantic Collector DTO
```

FastAPI 라우터는 `backend/api/v1/master/master_routor.py`,
APScheduler는 `backend/core/scheduler.py`에 위치합니다.
