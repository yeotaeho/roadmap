# causal_chain_service 청크 단위 upsert·commit 무네트워크 테스트
...
from domain.market_insight.hub.services.causal_chain_service import (  # noqa: E402
    REFINE_CHUNK,
    CausalChainRefineService,
)
...
class _FakeLlm:
    async def extract_causal_chain(self, text: str) -> dict:
        return {
            "macro_event": "금리 인상",
            "industry_impact": "투자 위축",
            "youth_chance": "공공 일자리 증가",
        }
...
def test_chunked_commit() -> None:
    n = 60
    svc = CausalChainRefineService.__new__(CausalChainRefineService)
